...
DEVICE_ON_TIME = "20:00"     # Counter=0 anchor time (row 0 = this time for all sensors)
# NOTE: We use row index (not Counter column) as the time reference.
# This avoids S5's Counter-reset bug where Counter jumped back to 0 mid-file.
...
# Baseline / enhancement policy
# - Your new request: use the *lowest* sensor baseline (median/mean) as a global baseline so every sensor has nonzero enhancement.
# - Safer default: keep per-sensor baselines for the S-calc inversion, but use the global-min baseline for *display* (imprint/assimilation/overlays).
BASELINE_STAT = "median"          # "median" or "mean"
BASELINE_MIN_POINTS = 3           # ignore sensors with < this many 1-min samples in the baseline window

USE_GLOBAL_MIN_BASELINE_FOR_DISPLAY = True
USE_GLOBAL_MIN_BASELINE_FOR_INVERSION = False   # set True if you want the S-calc inversion to also use the global-min baseline

# If True, residual assimilation only adds positive corrections (prevents 'holes' around sensors)
ASSIM_POSITIVE_ONLY = True
...
WIND_CSV    = "KSEA.2025-07-05.csv"
OUT_DIR     = "s_calc_outputs_777"
...
TARGET_POINTS_PER_FRAME = 6000

# Puff params search grid  (22×22×22 = 10,648 triplets)
SIGMA0_GRID = np.linspace(50, 800, 22).tolist()    # meters
K_GRID      = np.linspace(10, 1000, 22).tolist()   # m^2/s
TAU_GRID    = np.linspace(120, 3600, 22).tolist()   # seconds (1 hour max)
...
# ---------- Fallback Q estimator ----------
# When S-calc produces all zeros (data too coarse for conditional sampling),
# automatically switch to distance+wind-alignment weighted enhancement estimator.
SCALC_FALLBACK = True
SCALC_FALLBACK_IDW_POWER = 1.5      # inverse-distance weighting exponent
SCALC_FALLBACK_ALIGN_SCALE = 200.0  # sigmoid steepness for downwind alignment (meters)
...
FORCE_EXCLUDE_SENSORS: List[str] = ["S4"]   # e.g. ["S4"] if it dominates
WEIGHT_CAP_MIN = 0.2
WEIGHT_CAP_MAX = 5.0
...
def read_sensor_csv_1hz(path: str, anchor_start: pd.Timestamp) -> pd.DataFrame:
    """Read a sensor CSV and assign timestamps.

    Uses ROW INDEX (0, 1, 2, …) as the time counter — each row = 1 second.
    This avoids the Counter column, which can reset mid-file (observed in S5)
    and produce garbled timestamps. The Date/Time columns in these CSVs are
    also unreliable (e.g. '2105/74/8', '105:1:0').
    """
    ...
    df = df.dropna(subset=[pm_col]).copy()

    # Use row position as seconds-since-anchor (NOT Counter, which can reset)
    df = df.reset_index(drop=True)
    df["dt"] = anchor_start + pd.to_timedelta(df.index, unit="s")
...
def load_all_sensors_1min_raw_and_fit(t_anchor: pd.Timestamp, t_end: pd.Timestamp) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Return (wide_raw, wide_fit) at 1-min resolution.
    wide_raw: reindexed minute grid with NaNs preserved (no ffill/bfill)
    wide_fit: lightly interpolated + ffill/bfill for downstream modelling/visualization
    """
    series: Dict[str, pd.Series] = {}
    for fp in sorted(glob.glob(SENSOR_GLOB)):
        base = os.path.basename(fp)
    ...
    wide = pd.concat(series, axis=1).sort_index()
    full_idx = pd.date_range(t_anchor, t_end, freq="1min")
    wide_raw = wide.reindex(full_idx)  # keep NaNs (important for baseline validity)
    wide_fit = wide_raw.interpolate(limit=3).ffill().bfill()
    return wide_raw, wide_fit


def read_wind_1min_noaa(t0: pd.Timestamp, t1: pd.Timestamp) -> pd.DataFrame:
    """Read wind CSV and resample to 1-min.

    Supports two formats:
      (A) Original: columns DATE, wind_speed, wind_direction
      (B) MesoWest/A1526: comment header, columns Date_Time, wind_speed_set_1, wind_direction_set_1
    """
    # --- Detect format by peeking at the file ---
    with open(WIND_CSV, "r") as fh:
        first_line = fh.readline()

    if first_line.startswith("#") or "Date_Time" in first_line:
        # Format B: MesoWest / A1526 style
        ...
        with open(WIND_CSV, "r") as fh:
            for line in fh:
                if line.startswith("#"):
                    ...
                    break
        df = pd.read_csv(WIND_CSV, skiprows=n_skip, low_memory=False)
        df.columns = [str(c).strip() for c in df.columns]

        # Skip the units row if present (second row often has "Celsius", "m/s", etc.)
        if df.iloc[0].astype(str).str.contains("Celsius|m/s|Degrees|ug/m3|ppm|ppb|code", case=False).any():
            df = df.iloc[1:].reset_index(drop=True)

        if "Date_Time" not in df.columns:
            raise ValueError(f"Wind CSV (format B) must have Date_Time column. Found: {list(df.columns)}")

        # Parse timezone-aware timestamps, then convert to naive local time
        df["DATE"] = pd.to_datetime(df["Date_Time"], errors="coerce", utc=False)
        # If tz-aware, strip timezone (data is already in local time)
        if df["DATE"].dt.tz is not None:
            df["DATE"] = df["DATE"].dt.tz_localize(None)

        # Map column names
        ws_col = "wind_speed_set_1" if "wind_speed_set_1" in df.columns else "wind_speed"
        wd_col = "wind_direction_set_1" if "wind_direction_set_1" in df.columns else "wind_direction"
        df["wind_speed"] = pd.to_numeric(df[ws_col], errors="coerce")
        df["wind_direction"] = pd.to_numeric(df[wd_col], errors="coerce")
        # Already in m/s — no conversion needed
    else:
        # Format A: original (DATE, wind_speed, wind_direction)
        df = pd.read_csv(WIND_CSV, low_memory=False)
        df.columns = [str(c).strip() for c in df.columns]
        for col in ["DATE", "wind_speed", "wind_direction"]:
            if col not in df.columns:
                raise ValueError(f"Wind CSV (format A) must have DATE, wind_speed, wind_direction. Found: {list(df.columns)}")
        df["DATE"] = pd.to_datetime(df["DATE"], errors="coerce")
        df["wind_speed"] = pd.to_numeric(df["wind_speed"], errors="coerce")
        df["wind_direction"] = pd.to_numeric(df["wind_direction"], errors="coerce")

    df = df.dropna(subset=["DATE"]).copy()
    # Pad by 1 hour on each side so hourly data interpolates smoothly at window edges
    pad = pd.Timedelta(hours=1)
    df = df[(df["DATE"] >= t0 - pad) & (df["DATE"] <= t1 + pad)].copy()
    df = df.sort_values("DATE").set_index("DATE")

    wd = df["wind_direction"].to_numpy(dtype=float).copy()
    ...
    out["wind_speed"] = out["wind_speed"].interpolate(limit=120).ffill().bfill()
    out["wind_dir_from"] = out["wind_dir_from"].interpolate(limit=120).ffill().bfill()
...
def scalc_estimate_Q_proxy_profile(times: pd.DatetimeIndex,
    ...
    # ================================================================
    # S-calc diagnostic summary
    # ================================================================
    n_nonzero = int(np.sum(Q_proxy > 0))
    avg_win_samples = float(np.mean([
        min(T, idx + half + 1) - max(0, idx - half)
        for idx in rel_idx
    ]))
    n_bins_total = int(2 * SCALC_MAX_DEV_DEG / SCALC_DTHETA_DEG)
    samples_needed = int(SCALC_MIN_BINS * SCALC_MIN_SAMPLES_PER_BIN)

    print()
    print("=" * 60)
    print("  S-CALC DIAGNOSTICS")
    print("=" * 60)
    print(f"  Wind data resolution in S-calc window:")
    print(f"    Window length: {SCALC_WINDOW_MIN} minutes")
    print(f"    Avg samples per window: ~{avg_win_samples:.1f}")
    print(f"  Binning configuration:")
    print(f"    Bin width: {SCALC_DTHETA_DEG}° across ±{SCALC_MAX_DEV_DEG}° → {n_bins_total} bins")
    print(f"    Minimum: {SCALC_MIN_SAMPLES_PER_BIN} samples/bin × {SCALC_MIN_BINS} bins = {samples_needed} samples needed")
    if avg_win_samples < samples_needed:
        print(f"  ⚠ IMPOSSIBLE: only ~{avg_win_samples:.0f} samples available but ≥{samples_needed} needed!")
        print(f"    S-calc needs high-frequency wind data (≥1 Hz) so that many")
        print(f"    observations land in different wind-direction bins within each window.")
        print(f"    With your data cadence, almost all samples land in 1-2 bins,")
        print(f"    making it impossible to reconstruct the crosswind plume profile.")
    else:
        print(f"  ✓ Sufficient samples per window ({avg_win_samples:.0f} ≥ {samples_needed})")
    print(f"  Result: {n_nonzero} / {M} release times have nonzero Q_proxy")
    if n_nonzero == 0:
        print(f"  → Q_proxy is ALL ZEROS — S-calc produced nothing usable.")
        print(f"    The fallback estimator will be activated automatically.")
    elif n_nonzero < M:
        print(f"  → Partial: {M - n_nonzero} release times have zero Q_proxy")
    else:
        print(f"  → All release times have nonzero Q_proxy ✓")
    print("=" * 60)
    print()
...
# ================================================================
# FALLBACK Q_PROXY ESTIMATOR
# ================================================================
# Used when S-calc fails (data too coarse for conditional sampling).
# Instead of reconstructing the crosswind plume profile, it estimates
# relative emission strength as a weighted average of sensor enhancements.

def fallback_Q_proxy_from_enhancement(
        ...
        wind_speed: np.ndarray,
        wind_dir_from: np.ndarray,
        sensor_weights: np.ndarray) -> np.ndarray:
    """Estimate Q_proxy(t) from sensor enhancements when S-calc fails.

    For each release minute t:
      1. Get observed enhancement at each sensor
      2. Weight each sensor by:
         - sensor_weights (from robust weighting)
         - 1/distance^p  (closer sensors contribute more)
         - sigmoid alignment (downwind sensors contribute more)
      3. Q_proxy(t) = weighted mean of enhancements

    This gives a relative emission time profile — the grid search
    will find the absolute scale factor.
    """
    T, Ns = enh_ts_ns.shape
    M = len(rel_idx)
    dists = np.maximum(np.sqrt(sensor_xy[:, 0]**2 + sensor_xy[:, 1]**2), 10.0)
    d_weights = 1.0 / (dists ** float(SCALC_FALLBACK_IDW_POWER))

    print()
    print("=" * 60)
    print("  FALLBACK Q ESTIMATOR")
    print("=" * 60)
    print(f"  Method: distance + wind-alignment weighted sensor enhancement")
    print(f"  IDW power: {SCALC_FALLBACK_IDW_POWER}")
    print(f"  Alignment sigmoid scale: {SCALC_FALLBACK_ALIGN_SCALE} m")
    print(f"  Release times to estimate: {M}")
    print()
    print(f"  Sensor geometry:")
    for k, s in enumerate(sensors):
        print(f"    {s}: distance={dists[k]:.0f} m, "
              f"xy=({sensor_xy[k,0]:.0f}, {sensor_xy[k,1]:.0f}), "
              f"inv_dist_weight={d_weights[k]:.6f}, "
              f"sensor_weight={sensor_weights[k]:.3f}")

    Q_proxy = np.zeros(M, dtype=float)

    print()
    print(f"  Per-release-minute breakdown:")
    print(f"  {'min':>4s}  {'time':>20s}  {'wind_spd':>8s}  {'wind_dir':>8s}  "
          f"{'Q_proxy':>8s}  {'top_sensor':>10s}  {'top_enh':>8s}")
    print(f"  {'-'*4}  {'-'*20}  {'-'*8}  {'-'*8}  {'-'*8}  {'-'*10}  {'-'*8}")

    for j, idx in enumerate(rel_idx):
        # Observed enhancement at this minute (clamp negatives to 0)
        enh_j = np.where(
            np.isfinite(enh_ts_ns[idx]) & (enh_ts_ns[idx] > 0),
            enh_ts_ns[idx].astype(float), 0.0)

        # Wind direction → downwind unit vector
        wd_val = float(wind_dir_from[idx])
        theta_to = math.radians((wd_val + 180.0) % 360.0)
        ux, uy = math.sin(theta_to), math.cos(theta_to)

        # Alignment weight: sigmoid favoring sensors in the downwind direction
        align_w = np.zeros(Ns, dtype=float)
        for k in range(Ns):
            s_down, _ = project_to_wind(sensor_xy[k], ux, uy)
            align_w[k] = max(0.0, 1.0 / (1.0 + math.exp(
                -s_down / float(SCALC_FALLBACK_ALIGN_SCALE))))

        # Combined weights
        combined_w = sensor_weights * d_weights * align_w
        wsum = float(np.sum(combined_w))
        if wsum > 1e-12:
            combined_w /= wsum

        Q_proxy[j] = float(np.dot(combined_w, enh_j))

        # Find top contributing sensor for this minute
        contributions = combined_w * enh_j
        top_k = int(np.argmax(contributions))
        top_sensor = sensors[top_k]
        top_enh = float(enh_j[top_k])

        print(f"  {j+1:4d}  {str(times[idx]):>20s}  {wind_speed[idx]:8.2f}  "
              f"{wd_val:8.1f}  {Q_proxy[j]:8.4f}  {top_sensor:>10s}  {top_enh:8.2f}")

    # Smooth with moving average (same as S-calc)
    if int(SCALC_SMOOTH_MIN) >= 2 and Q_proxy.size >= 3:
        Q_raw = Q_proxy.copy()
        win = int(SCALC_SMOOTH_MIN)
        pad_n = win // 2
        qp = np.pad(Q_proxy, (pad_n, pad_n), mode='edge')
        Q_proxy = np.convolve(qp, np.ones(win)/win, mode='valid')
        print(f"\n  Smoothing: {win}-min moving average applied")
        print(f"    Before smoothing: min={Q_raw.min():.4f}, max={Q_raw.max():.4f}, "
              f"mean={Q_raw.mean():.4f}")
        print(f"    After smoothing:  min={Q_proxy.min():.4f}, max={Q_proxy.max():.4f}, "
              f"mean={Q_proxy.mean():.4f}")

    n_nonzero = int(np.sum(Q_proxy > 0))
    n_zero = M - n_nonzero
    print()
    print(f"  Summary:")
    print(f"    Nonzero Q_proxy: {n_nonzero} / {M} release times")
    if n_zero > 0:
        print(f"    ⚠ {n_zero} release times still have zero Q_proxy "
              f"(sensors had zero enhancement)")
    print(f"    Q_proxy range: [{Q_proxy.min():.4f}, {Q_proxy.max():.4f}]")
    print(f"    Q_proxy sum:   {Q_proxy.sum():.4f}")
    print(f"    Q_proxy mean:  {Q_proxy.mean():.4f}")
    print("=" * 60)
    print()

    return Q_proxy.astype(float)
...
def grid_search_forward_params(y_obs: np.ndarray,
                              ...
                              wind_dir_from: np.ndarray,
                              Q_proxy: np.ndarray,
                              sensor_weights: np.ndarray) -> Tuple[BestFit, np.ndarray, pd.DataFrame]:
    """Grid-search sigma0,K,tau (and apply scale) to best match observed enhancement given Q_proxy.
    Returns (best_fit, y_pred, objective_surface_df).
    """
    ...
    for sigma0 in SIGMA0_GRID:
        for K in K_GRID:
            for tau in TAU_GRID:
                ...
                records.append({"sigma0": float(sigma0), "K": float(K),
                                "tau": float(tau), "wrmse": float(e), "scale": float(a)})
    ...
    return best, best_y_pred, pd.DataFrame.from_records(records)


def _safe_r2(y_true: np.ndarray, y_pred: np.ndarray, floor: float = -10.0) -> float:
    """R² with sensible floor; NaN when SS_tot ≈ 0."""
    ss_res = float(np.sum((y_true - y_pred) ** 2))
    ss_tot = float(np.sum((y_true - np.mean(y_true)) ** 2))
    if ss_tot < 1e-8:
        return float('nan')
    return max(floor, 1.0 - ss_res / ss_tot)


def loo_cv_grid_search(y_obs, times, rel_idx, sensor_xy,
                       wind_speed, wind_dir_from,
                       Q_proxy, sensor_weights, sensors):
    """Leave-one-sensor-out cross-validation.

    For each fold:
      - Train on 5 sensors → grid search best (σ₀, K, τ, scale)
      - Predict on held-out sensor → compute test RMSE and R²
    """
    T, Ns = y_obs.shape
    records = []

    for k_out in range(Ns):
        train_idx = [i for i in range(Ns) if i != k_out]
        y_train = y_obs[:, train_idx]
        w_train = sensor_weights[train_idx]
        ...
        for sigma0 in SIGMA0_GRID:
            for K_v in K_GRID:
                for tau in TAU_GRID:
                    yhat0_all = predict_enh_from_q(
                        Q_proxy, times, rel_idx, sensor_xy,
                        wind_speed, wind_dir_from,
                        float(sigma0), float(K_v), float(tau))
                    a = fit_scale(y_train, yhat0_all[:, train_idx], w_train)
                    e = weighted_rmse(y_train, a * yhat0_all[:, train_idx], w_train)
                    cand = BestFit(float(sigma0), float(K_v), float(tau), float(e), float(a),
                                   Q_proxy.astype(float), (a * Q_proxy).astype(float), rel_idx)
                    if (best_fold is None) or (cand.rmse < best_fold.rmse):
                        best_fold = cand
                        best_yhat_all = a * yhat0_all

        y_test = y_obs[:, k_out]
        yhat_test = best_yhat_all[:, k_out]
        test_rmse = float(np.sqrt(np.mean((y_test - yhat_test) ** 2)))
        test_r2 = _safe_r2(y_test, yhat_test)

        records.append({
            "held_out": sensors[k_out],
            "sigma0": best_fold.sigma0, "K": best_fold.K, "tau": best_fold.tau,
            "train_wrmse": best_fold.rmse, "scale": best_fold.scale,
            "test_rmse": test_rmse, "test_r2": test_r2,
        })

        r2s = f"{test_r2:.3f}" if np.isfinite(test_r2) else "NaN (no variance)"
        print(f"    Fold {k_out+1}/{Ns}: held out {sensors[k_out]:>3s} → "
              f"σ₀={best_fold.sigma0:.0f}  K={best_fold.K:.0f}  τ={best_fold.tau:.0f}  "
              f"scale={best_fold.scale:.2f}  |  test RMSE={test_rmse:.3f}  R²={r2s}")

    return pd.DataFrame.from_records(records)


def characterize_objective_surface(surface_df: pd.DataFrame, tol_pct: float = 5.0) -> pd.DataFrame:
    """Return all triplets within tol_pct% of minimum WRMSE."""
    min_wrmse = surface_df["wrmse"].min()
    threshold = min_wrmse * (1.0 + tol_pct / 100.0)
    near = surface_df[surface_df["wrmse"] <= threshold].copy()
    return near.sort_values("wrmse").reset_index(drop=True)
...
def add_right_colorbar(
    ...
    vmin,
    vmax,
    gradient,  # dict like {0.0: "#2c7bb6", 0.5: "#f9d057", 1.0: "#d7191c"}
    caption="Enhancement (µg/m³)",
    right="14px",
    top="120px",
    height="220px",
    width="18px",
    ...
    # Build CSS gradient stops (0..1 -> 0..100%)
    stops = []
    for k in sorted(gradient.keys()):
        color = gradient[k]
        pct = 100.0 * float(k)
        stops.append(f"{color} {pct:.1f}%")
    css_grad = ", ".join(stops)

    # Tick labels (top is vmax, bottom is vmin)
    ticks = [vmin + (vmax - vmin) * i / (n_ticks - 1) for i in range(n_ticks)]
    ticks = ticks[::-1]  # display from high to low

    tick_html = ""
    for t in ticks:
        tick_html += f"<div style='line-height: 1; margin: 0; padding: 0;'>{t:.1f}</div>"
    ...
    <div style="
        ...
        font-size: 12px;
        ">
      <div style="font-weight: 600; margin-bottom: 6px; white-space: nowrap;">{caption}</div>

      <div style="display: flex; align-items: stretch; gap: 8px;">
        <div style="
            width: {width};
            height: {height};
            background: linear-gradient(to top, {css_grad});
            border-radius: 4px;
            border: 1px solid rgba(0,0,0,0.25);
        "></div>
...
def save_folium(frames_raw, labels, cmax: float, out_html: str):
    ...
    # Pick a consistent gradient (0..1 weights, because you normalized val/cmax)
    gradient = {
    ...
    # (your HeatMapWithTime call here; pass gradient=gradient)

    add_right_colorbar(
        m,
        vmin=0.0,
        vmax=float(cmax),            # your cmax (same as you use for val/cmax normalization)
        gradient=gradient,
        caption="Enhancement (µg/m³)",
        right="450px",
        top="350px",
    ...
    # Add a legend that maps 0..cmax in physical units (µg/m³)
    colors = [gradient[k] for k in sorted(gradient.keys()) if k != 0.0]
    colormap = cm.LinearColormap(colors=colors, vmin=0, vmax=float(cmax))
    colormap.caption = "Enhancement (µg/m³)"
    colormap.add_to(m)
...
def main():
    ...
    wide_raw_all, wide_all = load_all_sensors_1min_raw_and_fit(t_anchor, t1)
    ...
    wide_raw_all = wide_raw_all.loc[idx]
    wind_all = wind_all.loc[idx]

    # Analysis window
    wide = wide_all.loc[wide_all.index >= t0].copy()
    wide_raw = wide_raw_all.loc[wide_raw_all.index >= t0].copy()
    ...
    wide_raw_fit = wide_raw.reindex(times).interpolate(limit=3)  # keep NaNs for baseline validity
    wind_fit = wind.reindex(times).interpolate(limit=6).ffill().bfill()

    # Baseline (median/mean in [BASELINE_START, BASELINE_END))
    # Use raw (unfilled) minute data for baseline validity so sensors with missing baseline data don't get a fake baseline.
    base_win_raw = wide_raw_fit.loc[(wide_raw_fit.index >= b0) & (wide_raw_fit.index < b1), sensors].copy()
    if base_win_raw.empty:
        base_win_raw = wide_raw_fit[sensors].iloc[:5].copy()

    counts = base_win_raw.notna().sum(axis=0)

    if str(BASELINE_STAT).lower() == "mean":
        baselines_sensor = base_win_raw.mean(axis=0, skipna=True)
    else:
        baselines_sensor = base_win_raw.median(axis=0, skipna=True)

    # Require at least a few real points in baseline window
    baselines_sensor[counts < BASELINE_MIN_POINTS] = np.nan

    # Global-min baseline (your request): choose the lowest *valid* sensor baseline
    global_baseline = float(np.nanmin(baselines_sensor.to_numpy(dtype=float)))
    if not np.isfinite(global_baseline):
        # Fallback: take first few minutes of filled data
        tmp = wide_fit[sensors].iloc[:5].copy()
        if str(BASELINE_STAT).lower() == "mean":
            global_baseline = float(np.nanmin(tmp.mean(axis=0, skipna=True).to_numpy(dtype=float)))
        else:
    ...
    # What baseline is used where?
    baselines_display = pd.Series({s: global_baseline for s in sensors})
    baselines_inversion = baselines_display if USE_GLOBAL_MIN_BASELINE_FOR_INVERSION else baselines_sensor
    baselines = baselines_inversion  # downstream bookkeeping

    # Save baselines
    bdf = pd.DataFrame({
        "baseline_sensor": baselines_sensor,
        "baseline_used_inversion": baselines_inversion,
        "baseline_global_min": [global_baseline]*len(sensors),
        "n_points_in_baseline_window": counts,
    }, index=sensors)
    bdf.to_csv(os.path.join(OUT_DIR, "baseline_values.csv"))
    print("Saved:", os.path.join(OUT_DIR, "baseline_values.csv"))
    print(f"Baseline window = [{b0.strftime('%H:%M')}, {b1.strftime('%H:%M')})  |  STAT={BASELINE_STAT}")
    print(f"Global-min baseline = {global_baseline:.3f} ug/m3  |  used_inversion={USE_GLOBAL_MIN_BASELINE_FOR_INVERSION}  used_display={USE_GLOBAL_MIN_BASELINE_FOR_DISPLAY}")

    # Enhancement above baseline
    enh_inv = (wide_fit[sensors] - baselines_inversion[sensors]).clip(lower=0.0)
    enh_display = (wide_fit[sensors] - global_baseline).clip(lower=0.0) if USE_GLOBAL_MIN_BASELINE_FOR_DISPLAY else enh_inv

    enh = enh_inv  # keep name used later for diagnostics
    y_obs = enh_inv.to_numpy(dtype=float)
    y_obs_display = enh_display.to_numpy(dtype=float)
...
# Sensor XY in meters around source
    ...
    # -----------------
    # Check S-calc result → activate fallback if needed
    # -----------------
    q_method = "scalc"
    n_nonzero_scalc = int(np.sum(Q_proxy > 0))

    if n_nonzero_scalc == 0 and SCALC_FALLBACK:
        print()
        print("*" * 60)
        print("  S-calc Q_proxy is ALL ZEROS")
        print("  → Activating FALLBACK estimator")
        print("  (distance + wind-alignment weighted enhancement)")
        print("*" * 60)

        Q_proxy = fallback_Q_proxy_from_enhancement(
            times=times,
            rel_idx=rel_idx,
            enh_ts_ns=y_obs,
            sensor_xy=sensor_xy,
        ...
        # Save fallback Q_proxy
        fb_df = pd.DataFrame({
            "time": times[rel_idx],
            "Q_proxy_fallback": Q_proxy,
        })
        fb_df.to_csv(os.path.join(OUT_DIR, "fallback_Q_proxy.csv"), index=False)
        print("Saved:", os.path.join(OUT_DIR, "fallback_Q_proxy.csv"))

    elif n_nonzero_scalc == 0 and not SCALC_FALLBACK:
        print()
        print("⚠ S-calc Q_proxy is all zeros and SCALC_FALLBACK is disabled.")
        print("  The grid search will find scale=0 (no predictive power).")
        print("  Set SCALC_FALLBACK = True to use the fallback estimator.")
    else:
        print(f"\n✓ S-calc produced {n_nonzero_scalc}/{len(Q_proxy)} nonzero Q_proxy values — using S-calc result.\n")

    # -----------------
    # Forward-model diagnostics (before grid search)
    # -----------------
    print()
    print("=" * 60)
    print("  FORWARD-MODEL DIAGNOSTICS (pre-grid-search)")
    print("=" * 60)
    print(f"  Q_proxy method: {q_method}")
    print(f"  Q_proxy length: {Q_proxy.size}")
    print(f"  Q_proxy nonzero: {int(np.sum(Q_proxy > 0))} / {Q_proxy.size}")
    print(f"  Q_proxy range: [{Q_proxy.min():.4f}, {Q_proxy.max():.4f}]")
    print(f"  Q_proxy sum: {Q_proxy.sum():.4f}")
    if int(np.sum(Q_proxy > 0)) == 0:
        print(f"  ⚠ Q_proxy is ALL ZEROS — grid search will have scale=0!")
    else:
        # Probe forward model at mid-grid values
        sig_mid = float(np.median(SIGMA0_GRID))
        k_mid = float(np.median(K_GRID))
        tau_mid = float(np.median(TAU_GRID))
        yhat_probe = predict_enh_from_q(
            Q_proxy, times, rel_idx, sensor_xy, ws, wd,
            sig_mid, k_mid, tau_mid)
        max_pred = float(np.max(yhat_probe))
        a_probe = fit_scale(y_obs, yhat_probe, w_series.to_numpy(dtype=float))
        print(f"  Probe at mid-grid (σ₀={sig_mid:.0f}, K={k_mid:.0f}, τ={tau_mid:.0f}):")
        print(f"    max predicted enhancement (unscaled): {max_pred:.6f}")
        print(f"    fitted scale factor: {a_probe:.4f}")
        print(f"    max predicted enhancement (scaled): {max_pred * a_probe:.4f}")
        print(f"  Sensor distances and max observed enhancement:")
        dists_diag = np.sqrt(sensor_xy[:, 0]**2 + sensor_xy[:, 1]**2)
        for k, s in enumerate(sensors):
            obs_max = float(np.max(y_obs[:, k]))
            pred_max = float(np.max(yhat_probe[:, k]))
            print(f"    {s}: dist={dists_diag[k]:.0f} m, obs_max={obs_max:.2f}, "
                  f"pred_max(unscaled)={pred_max:.6f}")
    print("=" * 60)

    # -----------------
    # Fit forward plume parameters (sigma0,K,tau) + global scale on Q_proxy
    # -----------------
    n_triplets = len(SIGMA0_GRID) * len(K_GRID) * len(TAU_GRID)
    print(f"\n--- Grid search: {n_triplets} triplets "
          f"({len(SIGMA0_GRID)}×{len(K_GRID)}×{len(TAU_GRID)}) ---")
    print(f"  σ₀ range: [{min(SIGMA0_GRID)}, {max(SIGMA0_GRID)}] m  ({len(SIGMA0_GRID)} values)")
    print(f"  K  range: [{min(K_GRID)}, {max(K_GRID)}] m²/s  ({len(K_GRID)} values)")
    print(f"  τ  range: [{min(TAU_GRID)}, {max(TAU_GRID)}] s  ({len(TAU_GRID)} values)")

    best, y_pred, surface_df = grid_search_forward_params(
        y_obs=y_obs,
        times=times,
        rel_idx=rel_idx,
        sensor_xy=sensor_xy,
    ...
    print(f"    σ₀ = {best.sigma0:.2f} m  (initial plume width)")
    print(f"    K  = {best.K:.2f} m²/s  (eddy diffusivity)")
    print(f"    τ  = {best.tau:.2f} s  ({best.tau/60:.1f} min)  (exponential decay)")
    print(f"    scale = {best.scale:.4f}")
    print(f"  Weighted RMSE: {best.rmse:.4f} µg/m³")
    print(f"  Q_proxy → Q_profile via scale:")
    print(f"    Q_proxy range: [{best.Q_proxy.min():.4f}, {best.Q_proxy.max():.4f}]")
    print(f"    Q (scaled) range: [{best.Q_profile.min():.4f}, {best.Q_profile.max():.4f}]")

    # Check boundary hits
    boundary_hits = []
    for name, val, grid in [("σ₀", best.sigma0, SIGMA0_GRID),
                             ("K", best.K, K_GRID),
                             ("τ", best.tau, TAU_GRID)]:
        gmin, gmax = min(grid), max(grid)
        rng = gmax - gmin
        if val <= gmin + 0.01 * rng:
            boundary_hits.append(f"{name}={val:.0f} hit LOWER bound ({gmin})")
        elif val >= gmax - 0.01 * rng:
            boundary_hits.append(f"{name}={val:.0f} hit UPPER bound ({gmax})")
    if boundary_hits:
        print(f"  ⚠ BOUNDARY HITS:")
        for bh in boundary_hits:
            print(f"    {bh}")
        print(f"    → True optimum may lie outside the grid. Consider extending ranges.")
    else:
        print(f"  ✓ Best-fit is interior to grid (no boundary hits)")

    # Per-sensor fit quality + dispersion model evaluation metrics
    dists_res = np.sqrt(sensor_xy[:, 0]**2 + sensor_xy[:, 1]**2)

    # ── Standard dispersion model evaluation metrics (Chang & Hanna 2004) ──
    obs_flat = y_obs.flatten()
    pred_flat = y_pred.flatten()

    # Filter to positive observations for ratio-based metrics
    valid_mask = (obs_flat > 0.5) & np.isfinite(obs_flat) & np.isfinite(pred_flat)
    O_v = obs_flat[valid_mask]
    P_v = pred_flat[valid_mask]

    # FAC2: fraction within factor of 2 (0.5 ≤ P/O ≤ 2.0)
    if len(O_v) > 0:
        ratios = P_v / np.maximum(O_v, 1e-12)
        fac2_all = float(np.mean((ratios >= 0.5) & (ratios <= 2.0)))
    else:
        fac2_all = float('nan')

    # Fractional Bias: FB = 2(Ō-P̄)/(Ō+P̄), range [-2,+2], 0 = perfect
    O_mean = float(np.mean(O_v)) if len(O_v) > 0 else 1e-12
    P_mean = float(np.mean(P_v)) if len(P_v) > 0 else 0.0
    fb_all = 2.0 * (O_mean - P_mean) / max(O_mean + P_mean, 1e-12)

    # NMSE: normalized mean square error = (O-P)²̄ / (Ō·P̄), ≥0, 0 = perfect
    nmse_all = float(np.mean((O_v - P_v)**2)) / max(O_mean * P_mean, 1e-12) if len(O_v) > 0 else float('nan')

    # MG: geometric mean bias = exp(ln(O)̄ - ln(P)̄), 1 = perfect
    pos_mask = (O_v > 0.5) & (P_v > 0.5)
    if np.sum(pos_mask) > 5:
        mg_all = float(np.exp(np.mean(np.log(O_v[pos_mask])) - np.mean(np.log(P_v[pos_mask]))))
    else:
        mg_all = float('nan')

    # Index of Agreement (Willmott 1981): d = 1 - Σ(P-O)² / Σ(|P-Ō|+|O-Ō|)²
    # Always 0–1, 1 = perfect
    O_bar = float(np.mean(obs_flat))
    ioa_denom = float(np.sum((np.abs(pred_flat - O_bar) + np.abs(obs_flat - O_bar))**2))
    ioa_all = 1.0 - float(np.sum((pred_flat - obs_flat)**2)) / max(ioa_denom, 1e-12)
    ioa_all = max(0.0, ioa_all)

    # Pearson r (overall)
    ss_tot_all = float(np.sum((obs_flat - O_bar)**2))
    pearson_full = float(np.corrcoef(obs_flat, pred_flat)[0, 1]) if ss_tot_all > 1e-8 else float('nan')
    ...
    print(f"  Fractional Bias (FB):     {fb_all:+.4f}   (±0 = unbiased; >0 underpredicts)")
    print(f"  Norm. Mean Sq. Error:     {nmse_all:.4f}   (<4.0 = acceptable)")
    if np.isfinite(mg_all):
        print(f"  Geometric Mean Bias (MG): {mg_all:.4f}   (1.0 = perfect)")
    print(f"  Index of Agreement (IOA): {ioa_all:.4f}   (0–1, >0.5 = good)")
    print(f"  Pearson r:                {pearson_full:.4f}")
    print(f"  RMSE:                     {rmse_all:.4f} µg/m³")
    print(f"  Obs mean: {O_mean:.2f}   Pred mean: {P_mean:.2f}   "
          f"Valid pairs: {len(O_v)} (obs > 0.5 µg/m³)")

    print(f"\n  Per-sensor evaluation:")
    print(f"  {'sensor':>6s}  {'dist_m':>7s}  {'obs_max':>8s}  {'pred_max':>9s}  "
          f"{'obs_mean':>8s}  {'pred_mean':>9s}  {'RMSE':>8s}  {'FAC2':>6s}  "
          f"{'FB':>7s}  {'IOA':>6s}  {'r':>6s}")
    print(f"  {'-'*6}  {'-'*7}  {'-'*8}  {'-'*9}  {'-'*8}  {'-'*9}  "
          f"{'-'*8}  {'-'*6}  {'-'*7}  {'-'*6}  {'-'*6}")
    ...
    for k, s in enumerate(sensors):
        obs_k = y_obs[:, k]
        pred_k = y_pred[:, k]
        rmse_k = float(np.sqrt(np.mean((obs_k - pred_k)**2)))

        # Per-sensor FAC2
        vm = (obs_k > 0.5) & np.isfinite(obs_k) & np.isfinite(pred_k)
        if np.sum(vm) > 0:
            rat_k = pred_k[vm] / np.maximum(obs_k[vm], 1e-12)
            fac2_k = float(np.mean((rat_k >= 0.5) & (rat_k <= 2.0)))
        else:
            fac2_k = float('nan')

        # Per-sensor FB
        om_k = float(np.mean(obs_k[vm])) if np.sum(vm) > 0 else 1e-12
        pm_k = float(np.mean(pred_k[vm])) if np.sum(vm) > 0 else 0.0
        fb_k = 2.0 * (om_k - pm_k) / max(om_k + pm_k, 1e-12)

        # Per-sensor IOA
        ob_k = float(np.mean(obs_k))
        ioa_den_k = float(np.sum((np.abs(pred_k - ob_k) + np.abs(obs_k - ob_k))**2))
        ioa_k = max(0.0, 1.0 - float(np.sum((pred_k - obs_k)**2)) / max(ioa_den_k, 1e-12))

        # Per-sensor Pearson r
        ss_tot_k = float(np.sum((obs_k - np.mean(obs_k))**2))
        r_k = float(np.corrcoef(obs_k, pred_k)[0, 1]) if ss_tot_k > 1e-8 else float('nan')

        sensor_fac2_list.append(fac2_k)
        sensor_ioa_list.append(ioa_k)
        sensor_r_list.append(r_k)

        fac2_str = f"{fac2_k:6.3f}" if np.isfinite(fac2_k) else "   NaN"
        r_str = f"{r_k:6.4f}" if np.isfinite(r_k) else "   NaN"
        print(f"  {s:>6s}  {dists_res[k]:7.0f}  {obs_k.max():8.2f}  {pred_k.max():9.2f}  "
              f"{obs_k.mean():8.2f}  {pred_k.mean():9.2f}  {rmse_k:8.2f}  {fac2_str}  "
              f"{fb_k:+7.3f}  {ioa_k:6.3f}  {r_str}")

    valid_fac2 = [v for v in sensor_fac2_list if np.isfinite(v)]
    valid_ioa = [v for v in sensor_ioa_list if np.isfinite(v)]
    valid_r = [v for v in sensor_r_list if np.isfinite(v)]
    if valid_fac2:
        print(f"\n  Mean per-sensor FAC2: {np.mean(valid_fac2):.4f} ± {np.std(valid_fac2):.4f}")
        print(f"  Mean per-sensor IOA:  {np.mean(valid_ioa):.4f} ± {np.std(valid_ioa):.4f}")
        print(f"  Mean per-sensor r:    {np.mean(valid_r):.4f} ± {np.std(valid_r):.4f}")

    if best.scale == 0.0:
        print(f"\n  ⚠ scale=0 — forward model has NO predictive power!")
        print(f"    This means Q_proxy was all zeros (S-calc failed and fallback not active)")
        print(f"    or the forward model cannot match any sensor pattern.")
    print("=" * 60)

    # -----------------
    # Save objective surface
    # -----------------
    surface_df.to_csv(os.path.join(OUT_DIR, "objective_surface.csv"), index=False)
    print("\nSaved:", os.path.join(OUT_DIR, "objective_surface.csv"))

    # ================================================================
    # OBJECTIVE SURFACE CHARACTERIZATION (Issue 4a)
    # ================================================================
    min_wrmse = float(surface_df["wrmse"].min())
    max_wrmse = float(surface_df["wrmse"].max())
    threshold_5pct = min_wrmse * 1.05
    near_best = characterize_objective_surface(surface_df, tol_pct=5.0)
    near_best.to_csv(os.path.join(OUT_DIR, "near_optimal_triplets_5pct.csv"), index=False)
    n_near = len(near_best)
    n_total = len(surface_df)

    # Verify filter correctness
    if n_near > 0:
        filter_max = float(near_best["wrmse"].max())
        assert filter_max <= threshold_5pct + 1e-9, \
            f"BUG: near-optimal max {filter_max:.6f} exceeds threshold {threshold_5pct:.6f}"

    print()
    print("=" * 60)
    print("  OBJECTIVE SURFACE CHARACTERIZATION")
    print("=" * 60)
    print(f"  Total triplets evaluated: {n_total}")
    print(f"  Best WRMSE (global min): {min_wrmse:.4f} µg/m³")
    print(f"  Worst WRMSE (global max): {max_wrmse:.4f} µg/m³")
    print(f"  Relative spread: {100*(max_wrmse - min_wrmse)/max(min_wrmse,1e-12):.2f}%")
    print(f"  5% tolerance threshold: WRMSE ≤ {threshold_5pct:.4f}  (= 1.05 × {min_wrmse:.4f})")
    print(f"  Near-optimal triplets: {n_near} / {n_total} ({100.0*n_near/n_total:.1f}%)")
    if n_near > 0:
        nb = near_best
        print(f"    WRMSE range: [{nb['wrmse'].min():.4f}, {nb['wrmse'].max():.4f}]  "
              f"(verified ≤ {threshold_5pct:.4f})")
        print(f"    σ₀ range: [{nb['sigma0'].min():.0f}, {nb['sigma0'].max():.0f}] m")
        print(f"    K  range: [{nb['K'].min():.0f}, {nb['K'].max():.0f}] m²/s")
        print(f"    τ  range: [{nb['tau'].min():.0f}, {nb['tau'].max():.0f}] s")

    frac = n_near / n_total
    if frac > 0.20:
        surface_quality = "FLAT"
        print(f"  → Surface is FLAT ({frac*100:.1f}% near-optimal): "
              f"parameters poorly identified.")
    elif frac > 0.05:
        surface_quality = "moderately flat"
        print(f"  → Surface is moderately flat ({frac*100:.1f}% near-optimal): "
              f"limited identifiability.")
    else:
        surface_quality = "well-defined"
        print(f"  → Well-defined minimum ({frac*100:.1f}% near-optimal).")

    print(f"  Saved: {os.path.join(OUT_DIR, 'near_optimal_triplets_5pct.csv')}")
    print("=" * 60)

    # ================================================================
    # LEAVE-ONE-SENSOR-OUT CROSS-VALIDATION (Issue 4b / Issue 5)
    # ================================================================
    print()
    print("=" * 60)
    print("  LEAVE-ONE-SENSOR-OUT CROSS-VALIDATION")
    print("=" * 60)
    print(f"  {len(sensors)} folds — each fold holds out 1 sensor, trains on {len(sensors)-1}")
    print(f"  Grid: {n_total} triplets per fold")
    print()

    loo_results = loo_cv_grid_search(
        y_obs, times, rel_idx, sensor_xy, ws, wd, Q_proxy,
        w_series.to_numpy(dtype=float), sensors)
    loo_results.to_csv(os.path.join(OUT_DIR, "loo_cv_results.csv"), index=False)

    cv_mean_rmse = loo_results["test_rmse"].mean()
    cv_std_rmse = loo_results["test_rmse"].std()
    r2v = loo_results["test_r2"].dropna()
    cv_mean_r2 = float(r2v.mean()) if len(r2v) > 0 else float('nan')
    cv_std_r2 = float(r2v.std()) if len(r2v) > 1 else float('nan')
    n_nan = int(loo_results["test_r2"].isna().sum())

    print()
    print(f"  LOO-CV Summary:")
    print(f"    Test RMSE: {cv_mean_rmse:.3f} ± {cv_std_rmse:.3f} µg/m³")

    print(f"\n  Parameter spread across folds (primary purpose of LOO-CV):")
    print(f"    σ₀: [{loo_results['sigma0'].min():.0f}, {loo_results['sigma0'].max():.0f}] m  "
          f"(mean={loo_results['sigma0'].mean():.0f})")
    print(f"    K:  [{loo_results['K'].min():.0f}, {loo_results['K'].max():.0f}] m²/s  "
          f"(mean={loo_results['K'].mean():.0f})")
    print(f"    τ:  [{loo_results['tau'].min():.0f}, {loo_results['tau'].max():.0f}] s  "
          f"(mean={loo_results['tau'].mean():.0f})")

    print(f"\n  ── Context: Full-data fit vs LOO-CV ──")
    print(f"  Full-data FAC2:  {fac2_all:.4f}   IOA: {ioa_all:.4f}   r: {pearson_full:.4f}")
    if np.isfinite(cv_mean_r2):
        nan_note = f" ({n_nan} fold(s) NaN)" if n_nan else ""
        print(f"  LOO-CV R² (predict held-out sensor): {cv_mean_r2:.3f} ± {cv_std_r2:.3f}{nan_note}")
    else:
        print(f"  LOO-CV R² (predict held-out sensor): undefined (all {n_nan} folds NaN)")
    print(f"  Note: LOO-CV R² is negative because predicting an *unseen spatial")
    print(f"  location* from {len(sensors)-1} other sensors with an isotropic kernel is")
    print(f"  harder than predicting the mean. Standard dispersion model evaluation")
    print(f"  uses FAC2, FB, NMSE, and IOA (Chang & Hanna, 2004) — not R².")

    # LOO boundary check
    loo_bhits = []
    for _, row in loo_results.iterrows():
        for pn, pv, pg in [("σ₀", row["sigma0"], SIGMA0_GRID),
                            ("K", row["K"], K_GRID),
                            ("τ", row["tau"], TAU_GRID)]:
            gmin, gmax = min(pg), max(pg)
            rng = gmax - gmin
            if pv <= gmin + 0.01 * rng or pv >= gmax - 0.01 * rng:
                loo_bhits.append(f"fold={row['held_out']}: {pn}={pv:.0f}")
    if loo_bhits:
        print(f"  ⚠ {len(loo_bhits)} LOO fold-param(s) hit grid boundaries:")
        for bh in loo_bhits[:15]:
            print(f"      {bh}")
    else:
        print(f"  ✓ No LOO folds hit grid boundaries.")

    print(f"  Saved: {os.path.join(OUT_DIR, 'loo_cv_results.csv')}")
    print("=" * 60)

    # ================================================================
    # PLOTS: objective surface profiles + LOO-CV bar charts
    # ================================================================
    # Marginal profiles (min WRMSE vs each parameter, collapsing the other two)
    fig_s, axs = plt.subplots(1, 3, figsize=(14, 4))
    for ax, p, lab in zip(axs, ["sigma0", "K", "tau"],
                          ["σ₀ (m)", "K (m²/s)", "τ (s)"]):
        g = surface_df.groupby(p)["wrmse"].min()
        ax.plot(g.index, g.values, "o-", ms=3, lw=0.8)
        ax.axhline(min_wrmse, ls="--", c="red", lw=0.6, label="min")
        ax.axhline(threshold_5pct, ls=":", c="orange", lw=0.6, label="+5%")
        ax.set_xlabel(lab); ax.set_ylabel("min WRMSE (µg/m³)")
        ax.set_title(f"WRMSE vs {lab}")
        ax.legend(fontsize=7, frameon=False); ax.grid(True, alpha=0.25)
    fig_s.suptitle("Objective surface — marginal profiles", fontsize=11)
    fig_s.tight_layout()
    out_sf = os.path.join(OUT_DIR, "objective_surface_profiles.png")
    fig_s.savefig(out_sf, dpi=220); plt.close(fig_s)
    print("\nSaved:", out_sf)

    # LOO-CV bar charts
    fig_l, (axr, axp) = plt.subplots(1, 2, figsize=(12, 4))
    xp = np.arange(len(sensors))
    axr.bar(xp, loo_results["test_rmse"].values, color="steelblue",
            edgecolor="k", lw=0.5)
    axr.axhline(cv_mean_rmse, ls="--", c="red", lw=0.8,
                label=f"mean={cv_mean_rmse:.2f}")
    axr.set_xticks(xp); axr.set_xticklabels(loo_results["held_out"].values)
    axr.set_ylabel("Test RMSE (µg/m³)"); axr.set_title("LOO-CV: Test RMSE per fold")
    axr.legend(frameon=False); axr.grid(True, alpha=0.25, axis="y")

    # Right panel: best-fit K per fold (the one parameter with meaningful variation)
    axp.bar(xp, loo_results["K"].values, color="mediumpurple",
            edgecolor="k", lw=0.5)
    axp.axhline(best.K, ls="--", c="red", lw=0.8,
                label=f"full-data K={best.K:.0f}")
    axp.set_xticks(xp); axp.set_xticklabels(loo_results["held_out"].values)
    axp.set_ylabel("Best-fit K (m²/s)"); axp.set_title("LOO-CV: Parameter stability (K)")
    axp.legend(frameon=False); axp.grid(True, alpha=0.25, axis="y")
    fig_l.suptitle("Leave-One-Sensor-Out Cross-Validation", fontsize=11)
    fig_l.tight_layout()
    out_lf = os.path.join(OUT_DIR, "loo_cv_results.png")
    fig_l.savefig(out_lf, dpi=220); plt.close(fig_l)
    print("Saved:", out_lf)

    # Formatted strings for final summary
    cv_rmse_str = f"{cv_mean_rmse:.3f}±{cv_std_rmse:.3f}"

    print(f"\n=== BEST (Q_proxy [{q_method}] + centered plume forward fit) ===")
    print({
        "sigma0": best.sigma0,
        "K": best.K,
        "tau": best.tau,
        ...
        "grid_triplets": n_total,
        "n_near_optimal_5pct": n_near,
        "surface_quality": surface_quality,
        "boundary_hits": boundary_hits if boundary_hits else "none",
        "full_data_FAC2": round(fac2_all, 4),
        "full_data_FB": round(fb_all, 4),
        "full_data_IOA": round(ioa_all, 4),
        "full_data_Pearson_r": round(pearson_full, 4),
        "LOO_cv_RMSE": cv_rmse_str,
    })

    # Residual for optional assimilation (use DISPLAY enhancements so sensor imprint/assim matches what you see)
    residual_ts_ns = (y_obs_display - y_pred).astype(float)
    if ASSIM_POSITIVE_ONLY:
        residual_ts_ns = np.clip(residual_ts_ns, 0.0, np.inf)
    ...
    # Save Plotly
    obs_enh_overlay = enh_display.to_numpy(dtype=float)
    ...
    print("\nNotes:")
    print(f"  - Q(t) method: {q_method}")
    if q_method == "fallback":
        print("    S-calc failed (wind data too coarse for conditional sampling).")
        print("    Used distance + wind-alignment weighted enhancement fallback instead.")
        print("    This is expected when wind data cadence is > ~15 seconds.")
    else:
        print("    S-calc conditional sampling produced nonzero Q_proxy.")
    print(f"  - Grid: {n_total} triplets ({len(SIGMA0_GRID)}×{len(K_GRID)}×{len(TAU_GRID)})")
    print(f"  - Best WRMSE: {min_wrmse:.4f} µg/m³")
    print(f"  - FAC2 = {fac2_all:.4f}, IOA = {ioa_all:.4f}, FB = {fb_all:+.4f}, r = {pearson_full:.4f}")
    print(f"  - LOO-CV RMSE = {cv_rmse_str} (parameter spread, not predictive R²)")
    print(f"  - Near-optimal: {n_near}/{n_total} triplets within 5% ({surface_quality})")
    if boundary_hits:
        print(f"  - ⚠ Boundary hits: {boundary_hits}")
    print(f"  - New outputs:")
    print(f"      objective_surface.csv, near_optimal_triplets_5pct.csv")
    print(f"      objective_surface_profiles.png, loo_cv_results.csv, loo_cv_results.png")
    if q_method == "fallback":
        print(f"      fallback_Q_proxy.csv")
    print("  - If sensors still look blank, increase SENSOR_CLOUD_N / IMPRINT_ALPHA.")
    print("  - If decay too slow, reduce TAU_GRID upper bound / POST_EVENT_DECAY_TAU.")
