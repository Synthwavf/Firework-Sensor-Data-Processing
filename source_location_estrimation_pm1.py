#!/usr/bin/env python3
"""
PM1 Source Localization — Backward Lagrangian Stochastic Particle Dispersion

Inputs:
  - Data_1.csv through Data_6.csv (PM1 sensor data, S4 excluded)
...
Method:
  1. For each sensor × each time window:
       ...
       Weight by observed net PM1 at that sensor/window
...
Same method as source_location_estimation_pm25.py, applied to PM1_Env
(column 16) instead of PM2.5_Env (column 17).
...
OUTPUT_DIR = 'C:/Users/EricY/OneDrive/Desktop/Firework/Fireworks_Analysis/outputs_pm1'
...
MIN_NET_PM1   = 0.5   # minimum net PM1 to include a window (µg/m³)
...
def load_sensor_data(data_dir):
    """
    Load PM1 sensor data from CSV files.
    Returns dict: {sensor_id: (counter_array, pm1_array)}
    ...
    for sid in [1, 2, 3, 5, 6]:
        ...
        sensors[sid] = (counter_arr, pm1_arr)
...
def run_backward_lagrangian(sensors, wind_times, wind_speeds, wind_dirs):
    ...
    for sid, (counter, pm1) in sensors.items():
        ...
        for c_start, c_end in windows:
            # Get observed net PM1 in this window
            mask = (counter >= c_start) & (counter < c_end)
            if np.sum(mask) < 10:
                continue

            net_pm1 = np.mean(pm1[mask]) - background
            net_pm1 = max(net_pm1, 0.1)

            if net_pm1 < MIN_NET_PM1:
                continue
            ...
            for step in range(N_STEPS):
                ...
                np.add.at(footprint, (xi[valid], yi[valid]),
                          net_pm1 * residence_weight / N_PARTICLES)
...
def make_figure(est_x, est_y, footprint, grid_x, grid_y,
    ...
    conc = {}
    for sid, (counter, pm1) in sensors.items():
        bg = np.mean(pm1[(counter >= BG_START) & (counter <= BG_END)])
        conc[sid] = np.mean(pm1[(counter >= COUNTER_START) & (counter <= COUNTER_END)]) - bg
    ...
    ax.set_title(f'PM1 Concentration-Weighted Backward Trajectory Density\n'
...
if __name__ == '__main__':
    ...
    fig_path = os.path.join(OUTPUT_DIR, 'source_localization_result_pm1.png')
