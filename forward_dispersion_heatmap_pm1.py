#!/usr/bin/env python3
"""
PM1 Forward Dispersion Heatmap — Gaussian Puff Lagrangian Particle Model

Simulates the time-evolving spread of fireworks-generated PM1 from the
estimated source location (47.6389°N, 122.3352°W) using the same Lagrangian
...
Purpose:
  ...
  PM1 (sub-micron particles) represent the ultrafine fraction of fireworks
  aerosol.  Because these smaller particles have negligible gravitational
  settling, they remain airborne longer and diffuse more uniformly than
  PM2.5, making the near-field concentration gradient even more relevant
  for spectator exposure assessment.

Method:
  Same as forward_dispersion_heatmap_pm25.py, with:
    - Source at 47.6389°N, 122.3352°W (PM1 backward estimate)
    - PM1 sensor column (index 16) for validation overlay
    - Zero gravitational settling (sub-micron particles)

Inputs:
  - Data_1.csv through Data_6.csv (PM1 sensor data for validation overlay)
...
OUTPUT_DIR = os.path.join(DATA_DIR, 'outputs_pm1', 'forward_dispersion_pm1')
...
# 2D formulation: no vertical coordinate.  The old z<50 m breathing-zone
# filter is replaced by residence-time decay TAU_RES.
...
TAU_RES    = 500     # vertical-escape residence time (s)
...
def load_sensor_pm1(data_dir):
    """
    Load PM1 sensor data for validation overlay.
    Returns dict: {sensor_id: (time_seconds_array, net_pm1_array)}
    ...
    for sid in [1, 2, 3, 5, 6]:
        ...
        with open(filepath, 'r') as f:
            ...
            for i, row in enumerate(reader):
                ...
                try:
                    ...
                    pm1_values.append(float(row[16]))  # PM1_Env column
                except (ValueError, IndexError):
                    continue

        counter_arr = np.array(counters)
        pm1_arr = np.array(pm1_values)
        ...
        background = np.median(pm1_arr[bg_mask])

        # Convert counter to time
        time_seconds = T0_SECONDS + counter_arr * COUNTER_DT
        net_pm1 = np.maximum(pm1_arr - background, 0)

        sensors[sid] = (time_seconds, net_pm1)
...
def run_forward_dispersion(wind_times, wind_speeds, wind_dirs):
    """
    Forward Lagrangian particle dispersion from the estimated PM1 source.
    ...
    sim_start = SNAPSHOT_TIMES_PDT[0]
    sim_end   = SNAPSHOT_TIMES_PDT[-1] + 60
    ...
    while t <= sim_end:
        ...
        for snap_t in SNAPSHOT_TIMES_PDT:
            if abs(t - snap_t) < DT / 2 and snap_t not in snapshots:
                ...
                if n_active > 0:
                    # Residence-time weighting replaces z < 50 m filter.
...
def make_multipanel_figure(snapshots, grid_x, grid_y, sensor_data,
    ...
    for idx, snap_t in enumerate(SNAPSHOT_TIMES_PDT):
        ...
        # Sensor markers with observed PM1
    ...
    cbar.set_label('Relative PM$_{1}$ Concentration\n(Forward Dispersion Model)',
                   fontsize=11)

    fig.suptitle(
        'PM$_{1}$ Forward Dispersion Heatmap — Fireworks Plume Evolution\n'
...
def make_individual_snapshots(snapshots, grid_x, grid_y, sensor_data,
    ...
    for snap_t in SNAPSHOT_TIMES_PDT:
        ...
        time_str = seconds_to_pdt_str(snap_t)
        ax.set_title(f'PM$_{{1}}$ Forward Dispersion — {time_str} PDT\n'
                     f'Source: {SOURCE_LAT:.4f}°N, {abs(SOURCE_LON):.4f}°W',
        ...
        plt.colorbar(im, ax=ax,
                     label='Relative PM$_{1}$ concentration',
                     shrink=0.8)
        ...
        plt.tight_layout()
        fname = f'pm1_dispersion_{time_str.replace(":", "")}.png'
...
if __name__ == '__main__':
    print("=" * 60)
    print("PM1 Forward Dispersion Heatmap")
    ...
    print("\nLoading PM1 sensor data for overlay...")
    sensor_data = load_sensor_pm1(DATA_DIR)
    ...
    make_multipanel_figure(
        ...
        wind_times, wind_speeds, wind_dirs,
        os.path.join(OUTPUT_DIR, 'pm1_forward_dispersion_heatmap.png')
    )
    ...
    print("\n" + "=" * 60)
    print("DONE — PM1 Forward Dispersion Heatmap")
    print("=" * 60)
