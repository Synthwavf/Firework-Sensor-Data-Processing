...
OUT_DIR = "calibrated_outputs_with_baseline"
...
def main():
   ...
   # 5) NO baseline subtraction: fit raw PM2.5 directly
    baselines = pd.Series(0.0, index=wide_fit.columns)   # keep for compatibility
    y = wide_fit.fillna(0.0)                              # raw observations, no subtraction
    # NOTE: do NOT clip, since we're not working with "enhancement"
    ...
    out_html = os.path.join(OUT_DIR, "calibrated_puff_plume_time_with_baseline.html")
    m.save(out_html)
